...
from .serializers import (BalanceCreateSerializer,
                          BalanceSerializer, BillSerializer,
                          Bill_adjSerializer, PaymentSerializer,
                          ServiceSerializer)
...
def balance(request):
    ...
    return render(request, 'report.html',
                  {'data': balance_data, 'bill': bill_data})


def bill(request):
    if request.method == 'POST':
        biil_obj = Bill.objects.get(
            account_id=request.POST.get('account_id'),
            period_id=request.POST.get('period_id'))
        obj = BillSerializer(biil_obj).data
        payments_obj = Payment.objects.filter(
            account_id=request.POST.get('account_id'),
            period_id=request.POST.get('period_id'))
        payments = PaymentSerializer(payments_obj, many=True).data
        services = Service.objects.all()
        services_data = ServiceSerializer(services, many=True).data
        return render(request, 'bill.html', {'data': obj,
                                             'bill_id': biil_obj.id, 'services': services_data,
                                             'payments': payments})
